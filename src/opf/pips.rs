    ...
    S: crate::basic::solver::Solve,
{
    ...
    while !converged && i < opt.max_it {
        ...
        let dgn_csc = CscMatrix::try_from_csc_data(
            dgn.nrows(),
            dgn.ncols(),
            dgn.col_offsets().to_vec(),
            dgn.row_indices().to_vec(),
            dgn_v.clone(),
        )
        .unwrap();
        let dhn_csc = CscMatrix::try_from_csc_data(
            dhn.nrows(),
            dhn.ncols(),
            dhn.col_offsets().to_vec(),
            dhn.row_indices().to_vec(),
            dhn_v.clone(),
        )
        .unwrap();
        ...
        let (h_new, g_new, dh_new, dg_new) = merge_constraints(
            &x, &hn_v, &gn_v, &dhn_csc, &dgn_csc, &ai, &bi_vec, &ae, &be_vec,
        );
        h = h_new;
        g = g_new;
        dh = dh_new;
