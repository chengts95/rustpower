...
pub struct PipsResult {
    ...
    pub lam_eq: Vec<f64>,
    pub mu_ineq: Vec<f64>,
    pub mu_lower: Vec<f64>,
    pub mu_upper: Vec<f64>,
    pub message: String,
...
pub fn pips_with_solver<F, GH, H, S>(
    ...
    opt: PipsOpt,
    solver: &mut S,
    v5: Option<&crate::new_opf::assembly::v5::symbolic::KKTSymbolicV5>,
) -> PipsResult
where
    F: Fn(&[f64]) -> (f64, Vec<f64>),
    GH: Fn(&[f64]) -> (Vec<f64>, Vec<f64>, CscMatrix<f64>, CscMatrix<f64>),
    ...
    S: crate::basic::solver::Solve,
{
    ...
    while !converged && i < opt.max_it {
        ...
        let (dx, dlam_n, dz_n, dmu_n) = solve_kkt_timed(
            &lxx,
            &lx,
            &dg,
            &dh,
...
fn solve_kkt_timed<S: crate::basic::solver::Solve>(
            ...
            let m_mat = if let Some(dh_ref) = dh {
                if merged_slacks {
                    ...
                    CscMatrix::try_from_csc_data(
                        lxx.nrows(),
                        lxx.ncols(),
                        lxx_mod.col_offsets().to_vec(),
                        lxx_mod.row_indices().to_vec(),
                        lxx_v,
                    )
                    .unwrap()
                } else {
    ...
    *total_kkt += t_kkt.elapsed();
    ...
    let res = if neq == 0 {
        ...
        let (mut ap, mut ai, mut ax) = (
            m_mat.col_offsets().to_vec(),
            m_mat.row_indices().to_vec(),
            m_mat.values().to_vec(),
        );
        solver
            .solve(&mut ap, &mut ai, &mut ax, &mut rhs, nx)
            .unwrap();
        let dx = rhs;
        ...
        let mut rhs = n_vec
            ...
            .collect::<Vec<_>>();
        let (mut ap, mut ai, mut ax) = (
            ab_ref.col_offsets().to_vec(),
            ab_ref.row_indices().to_vec(),
            ab_ref.values().to_vec(),
        );
        solver
...
fn f64_add_sparse(a: &CscMatrix<f64>, b: &CscMatrix<f64>) -> CscMatrix<f64> {
    let (nb, a_cp, a_ri, a_v, b_cp, b_ri, b_v) = (
        a.ncols(),
        a.col_offsets(),
        a.row_indices(),
        a.values(),
    ...
    for j in 0..nb {
        ...
        while ia < a_cp[j + 1] || ib < b_cp[j + 1] {
            let ra = if ia < a_cp[j + 1] {
                a_ri[ia]
            } else {
                usize::MAX
            };
            let rb = if ib < b_cp[j + 1] {
                b_ri[ib]
            } else {
                usize::MAX
            };
...
pub(crate) fn build_linear_constraints(
    nx: usize,
    ieq: &[usize],
    ilt: &[usize],
    igt: &[usize],
...
pub(crate) fn merge_constraints(
    ...
    dhn: &CscMatrix<f64>,
    dgn: &CscMatrix<f64>,
    ai: &Option<CscMatrix<f64>>,
    bi: &[f64],
    ae: &Option<CscMatrix<f64>>,
    be: &[f64],
) -> (
    Vec<f64>,
    Vec<f64>,
    ...
    let (h, dh) = match ai {
        Some(r) => {
            ...
            (
                [hn, &hlin].concat(),
                Some(hstack_csc(dhn, &transpose_f64(r))),
            )
        }
    ...
    let (g, dg) = match ae {
        Some(r) => {
            ...
            (
                [gn, &glin].concat(),
                Some(hstack_csc(dgn, &transpose_f64(r))),
            )
        }
...
fn hstack_csc(a: &CscMatrix<f64>, b: &CscMatrix<f64>) -> CscMatrix<f64> {
    ...
    let mut cp = vec![0usize; na + nb + 1];
    cp[..na + 1].copy_from_slice(a.col_offsets());
    for j in 0..nb {
        cp[na + j + 1] = a.nnz() + b.col_offsets()[j + 1];
    }
...
fn spmv_f64(a: &CscMatrix<f64>, x: &[f64]) -> Vec<f64> {
    ...
    for j in 0..a.ncols() {
        ...
        for idx in a.col_offsets()[j]..a.col_offsets()[j + 1] {
            y[a.row_indices()[idx]] += a.values()[idx] * xj;
        }
    }
    y
}

pub(crate) fn convergence_measures(
    g: &[f64],
    h: &[f64],
    lx: &[f64],
    lam: &[f64],
    ...
    let comp = if z.is_empty() {
        0.0
    } else {
        z.iter().zip(mu).map(|(a, b)| a * b).sum::<f64>() / (1.0 + xnorm)
    };
    let cost = (f - f0).abs() / (1.0 + f0.abs());
    (feas, grad, comp, cost)
}

pub(crate) fn step_size(z: &[f64], dz: &[f64], xi: f64) -> f64 {
    let mut a = 1.0f64;
    for k in 0..z.len() {
        if dz[k] < 0.0 {
            a = a.min(xi * z[k] / (-dz[k]));
...
pub(crate) fn matvec_add_to(out: &mut [f64], dg: &CscMatrix<f64>, v: &[f64]) {
    for j in 0..dg.ncols() {
        let vj = v[j];
        if vj == 0.0 {
            continue;
        ...
        for idx in dg.col_offsets()[j]..dg.col_offsets()[j + 1] {
            out[dg.row_indices()[idx]] += dg.values()[idx] * vj;
        }
    }
}

// Compatibility entry points; optimized drivers now live beside their strategies.
pub use crate::new_opf::interior_point::{
    pips_with_fused_assembly, pips_with_fused_assembly_v55, pips_with_fused_assembly_v56,
};
