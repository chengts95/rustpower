use crate::io::pandapower::{
    Bus, ExtGrid, Gen, Line, Load, Network, SGen, Shunt, Switch, SwitchType, Transformer,
};
use pyo3::prelude::*;
...
impl Network {
    ...
    pub fn from_pp_net(&mut self, net: Bound<'_, PyAny>) -> PyResult<()> {
        ...
        if let Ok(df) = net.getattr("line") {
            self.line = Some(self.extract_lines(py, df)?);
        }
        if let Ok(df) = net.getattr("trafo") {
            self.trafo = Some(self.extract_trafos(py, df)?);
        }
        if let Ok(df) = net.getattr("load") {
            self.load = Some(self.extract_loads(py, df)?);
        }
        if let Ok(df) = net.getattr("gen") {
            self.r#gen = Some(self.extract_gens(py, df)?);
        }
        if let Ok(df) = net.getattr("ext_grid") {
            self.ext_grid = Some(self.extract_ext_grids(py, df)?);
        }
        if let Ok(df) = net.getattr("shunt") {
            self.shunt = Some(self.extract_shunts(py, df)?);
        }
        if let Ok(df) = net.getattr("sgen") {
            self.sgen = Some(self.extract_sgens(py, df)?);
        }
        if let Ok(df) = net.getattr("switch") {
            self.switch = Some(self.extract_switches(py, df)?);
        }

        if let Ok(f_hz) = net.getattr("f_hz") {
            self.f_hz = f_hz.extract()?;
        }
        if let Ok(sn_mva) = net.getattr("sn_mva") {
            self.sn_mva = sn_mva.extract()?;
        }
...
impl Network {
    ...
    fn extract_buses(&self, py: Python<'_>, df: Bound<'_, PyAny>) -> PyResult<Vec<Bus>> {
        ...
        let names = if df.hasattr("name")? {
            Self::get_opt_str_vec(py, &df, "name")?
        } else {
            vec![None; index.len()]
        };
        let types = if df.hasattr("type")? {
            Self::get_opt_str_vec(py, &df, "type")?
        } else {
            vec![None; index.len()]
        };

        let zones_f = if df.hasattr("zone")? {
            Self::get_opt_float_vec(py, &df, "zone")?
        } else {
            vec![None; index.len()]
        };
        let zones: Vec<Option<i64>> = zones_f.into_iter().map(|z| z.map(|v| v as i64)).collect();

        let min_vm = if df.hasattr("min_vm_pu")? {
            Self::get_opt_float_vec(py, &df, "min_vm_pu")?
        } else {
            vec![None; index.len()]
        };
        let max_vm = if df.hasattr("max_vm_pu")? {
            Self::get_opt_float_vec(py, &df, "max_vm_pu")?
        } else {
            vec![None; index.len()]
        };
    ...
    fn extract_lines(&self, py: Python<'_>, df: Bound<'_, PyAny>) -> PyResult<Vec<Line>> {
        ...
        let in_service = Self::get_bool_vec(&df, "in_service")?;
        let parallel =
            Self::get_int32_vec(&df, "parallel").unwrap_or_else(|_| vec![1; from_bus.len()]);
        let max_i_ka = if df.hasattr("max_i_ka")? {
            Self::get_opt_float_vec(py, &df, "max_i_ka")?
        } else {
            vec![None; from_bus.len()]
        };
        let names = if df.hasattr("name")? {
            Self::get_opt_str_vec(py, &df, "name")?
        } else {
        ...
        Ok((0..from_bus.len())
            .map(|i| Line {
                ...
                r_ohm_per_km: r_ohm[i],
                x_ohm_per_km: x_ohm[i],
                c_nf_per_km: c_nf[i],
                g_us_per_km: g_us[i],
                in_service: in_service[i],
                parallel: parallel[i],
    ...
    fn extract_trafos(&self, py: Python<'_>, df: Bound<'_, PyAny>) -> PyResult<Vec<Transformer>> {
        ...
        let tap_pos = if df.hasattr("tap_pos")? {
            Self::get_opt_float_vec(py, &df, "tap_pos")?
        } else {
            vec![None; hv_bus.len()]
        };
        let tap_side = if df.hasattr("tap_side")? {
            Self::get_opt_str_vec(py, &df, "tap_side")?
        } else {
            vec![None; hv_bus.len()]
        };
        let tap_neutral = if df.hasattr("tap_neutral")? {
            Self::get_opt_float_vec(py, &df, "tap_neutral")?
        } else {
            vec![None; hv_bus.len()]
        };
        let tap_step_percent = if df.hasattr("tap_step_percent")? {
            Self::get_opt_float_vec(py, &df, "tap_step_percent")?
        } else {
            vec![None; hv_bus.len()]
        };
    ...
    fn extract_gens(&self, py: Python<'_>, df: Bound<'_, PyAny>) -> PyResult<Vec<Gen>> {
        ...
        let in_service = Self::get_bool_vec(&df, "in_service")?;
        let slack = if df.hasattr("slack")? {
            Self::get_bool_vec(&df, "slack")?
        } else {
            vec![false; bus.len()]
        };
        let names = if df.hasattr("name")? {
    ...
    fn extract_shunts(&self, _py: Python<'_>, df: Bound<'_, PyAny>) -> PyResult<Vec<Shunt>> {
        ...
        let q_mvar = Self::get_float_vec(&df, "q_mvar")?;
        let in_service = Self::get_bool_vec(&df, "in_service")?;
        let vn_kv = if df.hasattr("vn_kv")? {
            Self::get_float_vec(&df, "vn_kv")?
        } else {
            vec![110.0; bus.len()]
        };
    ...
    fn extract_switches(&self, py: Python<'_>, df: Bound<'_, PyAny>) -> PyResult<Vec<Switch>> {
        ...
        let et = if df.hasattr("et")? {
            Self::get_opt_str_vec(py, &df, "et")?
        } else {
            vec![None; bus.len()]
        };
        ...
        Ok((0..bus.len())
            .map(|i| Switch {
                ...
                element: element[i],
                closed: closed[i],
                et: SwitchType::from(et[i].as_deref().unwrap_or("b")),
                name: None,
                type_: None,
                z_ohm: 0.0,
            })
