...
fn test_v5_2_pips_ieee118() {
    ...
    println!(
        "V5.2: converged={} iter={} f={:.6}",
        res52.converged, res52.iterations, res52.f
    );
    assert!(res52.converged, "V5.2 should converge");
    assert!(
        (res52.f - 129662.9725).abs() < 1e-2,
        "V5.2 result mismatch: {}",
        res52.f
    );
}
...
fn test_v5_versions_vs_v4_kkt_breakdown() {
    ...
    let v5_sym = crate::new_opf::assembly::v5::symbolic::KKTSymbolicV5::build(&data);
    let v53_sym = v5_3_kernel::KKTSymbolicV5_3::build(&data);

    // --- 1. Baseline: V4 Lxx and opf_consfcn dg ---
    let lxx_v4 = crate::new_opf::assembly::v4::curvature::v4_rect_numeric_fill(
        &data,
        &v3c,
        x.as_slice(),
        &lam_eq,
    ...
    v5_2_kernel::fill_variable_columns(
        &v5_sym,
        &data,
        &v3c.y_transpose_idx,
        &x,
    ...
    v5_2_kernel::fill_constraint_columns(
        &v5_sym,
        &data,
        &v3c.y_transpose_idx,
        &gens_at_bus,
    ...
    let compare = |label: &str, candidate: &[f64]| {
        ...
        for i in 0..ref_vals.len() {
            ...
            if d > 1e-11 {
                if diff_count < 5 {
                    println!(
                        "{} Diff at nnz {}: ref={:.6e}, cand={:.6e} (err={:.3e})",
                        label, i, ref_vals[i], candidate[i], d
                    );
                }
        ...
        println!(
            "{} vs V4 KKT: diff_count={}, max_diff={:.3e}",
...
fn test_v5_5_evaluator_correctness() {
    ...
    // 2. V5.5 Evaluator
    let evaluator = crate::new_opf::evaluation::v5::nonlinear::V55Evaluator::new(&data);
    ...
    // Verify g
    for (a, b) in g_ref.iter().zip(g_test.iter()) {
        assert!((a - b).abs() < 1e-12, "g mismatch");
    }
    // Verify h
    for (a, b) in h_ref.iter().zip(h_test.iter()) {
        assert!((a - b).abs() < 1e-12, "h mismatch");
    }
    // Verify dgn
    for (a, b) in dgn_ref.values().iter().zip(dgn_v_test.iter()) {
        assert!((a - b).abs() < 1e-12, "dgn mismatch: ref {}, test {}", a, b);
    }
    // Verify dhn
    for (a, b) in dhn_ref.values().iter().zip(dhn_v_test.iter()) {
        assert!((a - b).abs() < 1e-12, "dhn mismatch: ref {}, test {}", a, b);
    }
    ...
    // Verify g
    for (a, b) in g_ref.iter().zip(g_test.iter()) {
        assert!((a - b).abs() < 1e-12, "g mismatch on rand");
    }
    // Verify h
    for (a, b) in h_ref.iter().zip(h_test.iter()) {
        assert!((a - b).abs() < 1e-12, "h mismatch on rand");
    }
    // Verify dgn
    for (a, b) in dgn_ref.values().iter().zip(dgn_v_test.iter()) {
        assert!(
            (a - b).abs() < 1e-12,
            "dgn mismatch on rand: ref {}, test {}",
            a,
            b
        );
    }
...
fn test_v5_3_pips_ieee118() {
    ...
    println!(
        "V5.3: converged={} iter={} f={:.6}",
        res53.converged, res53.iterations, res53.f
    );
    assert!(res53.converged, "V5.3 should converge");
    assert!(
        (res53.f - 129662.9725).abs() < 1e-2,
        "V5.3 result mismatch: {}",
        res53.f
    );
}
...
fn test_baseline_opf_ieee39_run() {
    ...
    println!(
        "Baseline OPF 39: converged={} iter={} f={:.6}",
...
fn test_exact_hessian_comparison_pegase9241() {
    ...
    let _h_v1 =
        crate::opf::hessian::opf_hessfcn(&base_data, x.as_slice(), &lam_eq, &mu_ineq, cost_mult);
    let h_v3 = v3_numeric_scalar::v3_scalar_numeric_fill(
        &base_data,
        &v3_cache,
        x.as_slice(),
        &lam_eq,
    ...
    let h_v4 = v4_numeric_rect::v4_rect_numeric_fill(
        &base_data,
        &v3_cache,
        x.as_slice(),
        &lam_eq,
    ...
    println!(
        "Total v4-vs-V3 diffs: {}, Max Err: {:.6e}",
        diff_v4_v3, max_err_v4_v3
    );
...
fn diag_hessian_breakdown_ieee118() {
    ...
    println!(
        "  FD gold standard for V4: worst ratio={:.2e} abs={:.4e} at ({},{})",
        worst_v4.3, worst_v4.2, worst_v4.0, worst_v4.1
    );
    assert!(
        worst_v4.3 < 1.0,
        "v4 Hessian disagrees with finite differences"
    );
}
...
fn test_v5_pips_matches_v4_ieee118() {
    ...
    println!(
        "V4: converged={} iter={} f={:.6}",
        r4.converged, r4.iterations, r4.f
    );
    println!(
        "V5: converged={} iter={} f={:.6}",
        r5.converged, r5.iterations, r5.f
    );
    assert!(r5.converged, "V5 must converge");
    assert_eq!(
        r4.iterations, r5.iterations,
        "V5 must take same iterations as V4"
    );
    assert!(
        (r4.f - r5.f).abs() < 1e-6,
        "V5 objective must match V4 (diff={:.3e})",
        (r4.f - r5.f).abs()
    );
    let mut max_dx = 0.0f64;
    for (a, b) in r4.x.iter().zip(r5.x.iter()) {
        max_dx = max_dx.max((a - b).abs());
    }
    println!("V5 vs V4 max |Δx| = {:.3e}", max_dx);
    assert!(
        max_dx < 1e-9,
        "V5 solution must match V4 (max|Δx|={:.3e})",
        max_dx
    );
}
...
#[test]
fn configurations_preserve_solver_entry_points() {
    use crate::new_opf::configurations::{self, Configuration};
    let net = load_ieee39();
    let data = NewOPFData::new(opf_data_from_network(&net));
    let initial = data.warm_x0();
    let options = || PipsOpt {
        cost_mult: 1e-4,
        ..Default::default()
    };
    type Entry = fn(&NewOPFData, Vec<f64>, Vec<f64>, Vec<f64>, PipsOpt) -> PipsResult;
    let entries: [(Configuration, Entry); 6] = [
        (Configuration::V4, configurations::pips),
        (Configuration::V5_0, configurations::pips_v5),
        (Configuration::V5_2, configurations::pips_v5_2),
        (Configuration::V5_3, configurations::pips_v5_3),
        (Configuration::V5_5, configurations::pips_v5_5),
        (Configuration::V5_6, configurations::pips_v5_6),
    ];
    for (configuration, entry) in entries {
        let (lower, upper) = data.bounds();
        let direct = entry(&data, initial.clone(), lower, upper, options());
        let selected = configuration.solve(&data, initial.clone(), options());
        assert!(selected.converged, "{configuration:?} did not converge");
        assert_eq!(selected.iterations, direct.iterations);
        assert_eq!(selected.x, direct.x);
        assert_eq!(selected.f, direct.f);
    }
    let (lower, upper) = data.bounds();
    let direct = crate::opf::pips::pips(
        |x| crate::opf::cost::opf_costfcn(&data, x),
        |x| {
            let (g, h, dg, dh) = crate::opf::constraints::opf_consfcn(&data, x);
            (h, g, dh, dg)
        },
        |x, l, m, _z, c| crate::opf::hessian::opf_hessfcn(&data, x, l, m, c),
        initial.clone(),
        lower,
        upper,
        options(),
    );
    let selected = Configuration::V1.solve(&data, initial, options());
