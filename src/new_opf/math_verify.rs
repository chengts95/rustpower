...
pub fn verify_hessian(
    ...
    for v in h_sum.values() {
        h_ee_vals.push(v.re);
    }
    let h_ee = CscMatrix::try_from_csc_data(
        nb,
        nb,
        h_sum.col_offsets().to_vec(),
        h_sum.row_indices().to_vec(),
        h_ee_vals,
    )
    .unwrap();

    let mut h_ef_vals = Vec::with_capacity(h_diff.nnz());
    for v in h_diff.values() {
        h_ef_vals.push(v.im);
    }
    let h_ef = CscMatrix::try_from_csc_data(
        nb,
        nb,
        h_diff.col_offsets().to_vec(),
        h_diff.row_indices().to_vec(),
        h_ef_vals,
    )
    .unwrap();
    ...
    for i in 0..nb {
        lam_v_conj[i] = (Complex64::new(lp[i], -lq[i]) * v[i]).conj();
    }
    let term2 = ybus * &lam_v_conj;
    let mut g_rect_eq = vec![Complex64::new(0.0, 0.0); nb];
    for i in 0..nb {
        g_rect_eq[i] = Complex64::new(lp[i], -lq[i]) * ibus[i].conj() + term2[i];
    }
    ...
    let (dSf_dVa, dSf_dVm, dSt_dVa, dSt_dVm, Sf, St) = crate::basic::dsbr_dv::dSbr_dV(
        &data.yf,
        &data.yt,
        &data.f_buses,
        &data.t_buses,
    ...
    let hf = crate::basic::d2sbr_dv2::d2ASbr_dV2(
        &dSf_dVa,
        &dSf_dVm,
        &Sf,
        &data.cf,
    ...
    let ht = crate::basic::d2sbr_dv2::d2ASbr_dV2(
        &dSt_dVa,
        &dSt_dVm,
        &St,
        &data.ct,
    ...
    let add_blocks = |coo: &mut CooMatrix<f64>,
                      blocks: (
        CscMatrix<f64>,
        CscMatrix<f64>,
        CscMatrix<f64>,
    ...
    for g in 0..ng {
        h_full_coo.push(
            2 * nb + g,
            2 * nb + g,
            cost_mult * 2.0 * data.cost_coeffs[g][0] * base * base,
        );
    }
