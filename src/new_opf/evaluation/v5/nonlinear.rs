...
impl V55Evaluator {
    pub fn new(data: &OPFData) -> Self {
        ...
        let map_branch = |ybr: &nalgebra_sparse::CscMatrix<Complex64>,
                          is_vm: bool,
                          col_off: usize|
         -> Vec<usize> {
        ...
        Self {
            dgn_cp: dgn.col_offsets().to_vec(),
            dgn_ri: dgn.row_indices().to_vec(),
            dhn_cp: dhn.col_offsets().to_vec(),
            dhn_ri: dhn.row_indices().to_vec(),
            va_to_dgn_p,
            va_to_dgn_q,
            vm_to_dgn_p,
            vm_to_dgn_q,
    ...
    fn fill_branch(
        &self,
        ybr: &nalgebra_sparse::CscMatrix<Complex64>,
        bus: &[usize],
        vs: &[Complex64],
        vnorm: &[Complex64],
        i_br: &[Complex64],
...
mod tests {
    ...
    fn test_v5_5_direct_fill_matches_consfcn_ieee118() {
        ...
        println!(
            "V5.5 vs opf_consfcn: |g|={:.2e} |h|={:.2e} |dgn|={:.2e} |dhn|={:.2e}",
            dg, dh, ddg, ddh
        );
        assert!(
            dg < 1e-12 && dh < 1e-12 && ddg < 1e-12 && ddh < 1e-12,
            "V5.5 direct-fill mismatch: g={:.2e} h={:.2e} dgn={:.2e} dhn={:.2e}",
            dg,
            dh,
            ddg,
            ddh
