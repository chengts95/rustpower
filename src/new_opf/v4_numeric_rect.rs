...
pub fn v4_rect_numeric_fill(
    ...
    for j in 0..nb {
        ...
        let out_va =
            unsafe { std::slice::from_raw_parts_mut(lxx_ptr.add(cache.lxx_cp[j] + nnz_j), nnz_j) };
        let out_av =
            unsafe { std::slice::from_raw_parts_mut(lxx_ptr.add(cache.lxx_cp[nb + j]), nnz_j) };
        let out_vv = unsafe {
            std::slice::from_raw_parts_mut(lxx_ptr.add(cache.lxx_cp[nb + j] + nnz_j), nnz_j)
        };
        ...
        for offset in 0..nnz_j {
            ...
            let (e_ij, f_ij, gav) = if i == j {
                ...
                let f = c_ij;
                let gav_val =
                    (j_unit * inv_vmag[j] * (v_s[j].conj() * y_ij.conj() * lam_v[i] - c_ji)).re;
                (e, f, gav_val)
            };
    ...
    for l in 0..nl {
        ...
        let wf = if let Some(z) = z_ineq {
            mu_f[l] / z[l]
        } else {
            0.0
        };
        let wt = if let Some(z) = z_ineq {
            mu_t[l] / z[nl + l]
        } else {
            0.0
        };
