...
pub fn v3_fused_numeric_fill(
    ...
    let _add_branch_h_rect =
        |f: usize, t: usize, y_ff: Complex64, y_ft: Complex64, mu: f64, _y_indices: &[usize; 4]| {
            if mu == 0.0 {
                return;
            }
    ...
    let (d_sf_d_va, d_sf_d_vm, d_st_d_va, d_st_d_vm, sf, st) = crate::basic::dsbr_dv::dSbr_dV(
        &data.yf,
        &data.yt,
        &data.f_buses,
        &data.t_buses,
    ...
    let _hf = crate::basic::d2sbr_dv2::d2ASbr_dV2(
        &d_sf_d_va,
        &d_sf_d_vm,
        &sf,
        &data.cf,
    ...
    let _ht = crate::basic::d2sbr_dv2::d2ASbr_dV2(
        &d_st_d_va,
        &d_st_d_vm,
        &st,
        &data.ct,
    ...
    for j in 0..nb {
        for idx in y_cp[j]..y_cp[j + 1] {
            ...
            let haa =
                m_i[0] * (hrr * m_j[0] + hef * m_j[2]) + m_i[2] * (hfe * m_j[0] + hii * m_j[2]);
            let hav =
                m_i[0] * (hrr * m_j[1] + hef * m_j[3]) + m_i[2] * (hfe * m_j[1] + hii * m_j[3]);
            let hva =
                m_i[1] * (hrr * m_j[0] + hef * m_j[2]) + m_i[3] * (hfe * m_j[0] + hii * m_j[2]);
            let hvv =
                m_i[1] * (hrr * m_j[1] + hef * m_j[3]) + m_i[3] * (hfe * m_j[1] + hii * m_j[3]);
    ...
    for g in 0..ng {
        lxx_vals[cache.lxx_diag_ptrs[2 * nb + g]] =
