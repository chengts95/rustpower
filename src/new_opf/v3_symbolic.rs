...
impl V3SymbolicCache {
    pub fn analyze(data: &OPFData) -> Self {
        ...
        let find_l = |r: usize, c: usize| -> usize {
            ...
            l_ri[range.clone()].binary_search(&r).map(|pos| range.start + pos).expect("Lxx missing element!")
        };
        
        let find_sparse = |mat: &CscMatrix<Complex64>, r: usize, c: usize| -> usize {
            let range = mat.col_offsets()[c]..mat.col_offsets()[c + 1];
            mat.row_indices()[range.clone()].binary_search(&r).map(|pos| range.start + pos).expect("Sparse find failed")
        };

        for l in 0..nl {
            let f = data.f_buses[l];
            let t = data.t_buses[l];
            br_to_y_indices[l] = [ find_sparse(ybus, f, f), find_sparse(ybus, f, t), find_sparse(ybus, t, f), find_sparse(ybus, t, t) ];
        ...
        Self { 
            nb, nx, y_to_lxx, br_to_y_indices, br_to_lxx, br_to_yf_idx, br_to_yt_idx, 
