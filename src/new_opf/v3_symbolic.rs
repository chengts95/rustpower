    ...
    pub fn analyze(data: &OPFData) -> Self {
        ...
        for j in 0..nb {
            for idx in y_cp[j]..y_cp[j + 1] {
                ...
                let transpose_idx = y_ri[range.clone()]
                    .binary_search(&j)
                    .map(|pos| range.start + pos)
                    .expect("Transpose missing");
        ...
        for l in 0..nl {
            ...
            for ni in 0..2 {
                for nj in 0..2 {
                    ...
                    let c = nodes[nj];
                    let base = (ni * 2 + nj) * 4;
                    ptrs[base + 0] = find_l(r, c);
                    ptrs[base + 1] = find_l(r, nb + c);
                    ptrs[base + 2] = find_l(nb + r, c);
                    ptrs[base + 3] = find_l(nb + r, nb + c);
                }
            }
            br_to_lxx[l] = ptrs;
        }
...
impl KKTSymbolicCache {
    ...
    pub fn analyze(lxx_cache: &V3SymbolicCache, data: &OPFData) -> Self {
        ...
        let mut ieq = Vec::new();
        for i in 0..nx {
            if (xmax[i] - xmin[i]).abs() <= f64::EPSILON {
                ieq.push(i);
            }
        }
        let neqlin = ieq.len();
        ...
        let dg_final = match ae {
            Some(r) => {
                ...
                let nnza = dgn.nnz();
                for j in 0..rt.ncols() {
                    cp[dgn.ncols() + j + 1] = nnza + rt.col_offsets()[j + 1];
                }
        ...
        let dummy_lxx = CscMatrix::try_from_csc_data(
            nx,
            nx,
            lxx_cache.lxx_cp.clone(),
            lxx_cache.lxx_ri.clone(),
            vec![1.0; lxx_cache.lxx_ri.len()],
        )
        .unwrap();
        let kkt =
            crate::opf::pips::build_saddle_point(&dummy_lxx, &Some(dg_final.clone()), nx, neq);
        ...
        let lxx_to_kkt = (0..nx)
            .flat_map(|j| {
                (lxx_cache.lxx_cp[j]..lxx_cache.lxx_cp[j + 1])
                    .map(move |off| find_k(lxx_cache.lxx_ri[off], j))
            })
            .collect();
