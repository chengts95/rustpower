...
| 3 | Exact-LM driver (sparse factorization via the existing `rsparse` / KLU solver stack — the flat CSC is built precisely so a direct sparse solver consumes it) | convergence window reproduced (α ∈ [1.15, 1.2]: only exact LM converges); infeasibility stall beyond nose |
