...
use rustpower::prelude::{DataOps, PowerFlow, PowerGrid};
...
fn main() {
    ...
    let archive_res = world
        .get_resource::<ArchiveSnapshotRes>()
        .expect("Missing ArchiveSnapshotRes");
    ...
    let arrow_snap = WorldArrowSnapshot::from_world_reg(world, output_reg)
        .expect("Failed to create Arrow snapshot");
    ...
    println!(
        "Evaluation: This format allows columnar storage of ECS components. Each archetype/component group becomes a Parquet file, which is ideal for large-scale analysis in Pandas/Polars."
    );
}
