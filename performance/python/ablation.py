...
STAGES = {
    'AUG-FS-upper': ('A0', '全J裁剪 + 上三角COO（不用V4）'),
    'AUG-COO-upper': ('A1', 'V4 + 上三角COO'),
    'AUG-upper': ('A2', '原triu直填'),
    'AUG-operator': ('A3', '算子triu直填'),
}
TIME_FIELDS = ['build_ms', 'solve_ms', 'total_execution_ms', 'j_or_aug_fill_ms',
               ...
               'solver_setup_ms', 'solver_numeric_ms', 'solver_backsolve_ms']
...
def export(root, primary, primary_env, ablation, ablation_env, methods, cases, backends, backend_env):
    folder = root / 'data'
    folder.mkdir(exist_ok=True)
    raw = []
    for suite, records, env in [('lm-assembly', primary, primary_env), ('lm-ablation', ablation, ablation_env), ('lm-solvers', backends, backend_env)]:
    ...
    for row in stage_rows:
        baseline = next(r for r in stage_rows if r['case'] == row['case'] and r['method'] == 'AUG-FS-upper')
        independent = row['method'] in ('AUG-FS-upper', 'AUG-COO-upper')
    ...
    manifests = {name: {k: v for k, v in env.items() if k != 'source_diff'}
                 for name, env in [('lm-assembly', primary_env), ('lm-ablation', ablation_env), ('lm-solvers', backend_env)]}
    (folder / 'environment.json').write_text(json.dumps(manifests, indent=2, ensure_ascii=False) + '\n')
    backend_rows = backend_summaries(backends, cases)
    write_csv(folder / 'solvers.csv', backend_rows)
    write_latex_backends(root, backend_rows)
...
def markdown(rows, cases):
    lines = ['## 消融：从上三角COO到KKT直接填充', '',
             '独立入口`lm-ablation`运行现有四条驱动路径，各预热1次、测量7次。四条路径使用相同增广上三角、相同初值与LM参数；A0按解析公式计算全J，不使用V4或KktPattern；A1–A3使用KktPattern，并复用QDLDL。', '',
             '- A0：解析公式计算全J并裁剪，然后COO组装上三角并转CSC，不使用V4。',
             '- A1：V4计算保留的独立J，然后COO组装上三角并转CSC。',
             '- A2：原fill_jt_rows直接计算并写入最终Jᵀ块，消除独立J缓冲区和COO转换。',
             '- A3：JacobianOperator直接填入同一Jᵀ块，复用V3/V4的边导数关系，并读取当前Ybus。', '',
             '导数计算没有被消除：A2/A3把导数求值和最终矩阵写入合并，所以“独立J评估”列为0，但“直接Jᵀ评估与填充”仍有耗时。COO列包含三元组写入、μ及−I对角、转CSC、结构检查和数值复制；μ/右端列在A0/A1只计右端，在A2/A3还计μ更新。准备合计逐轮求和后取中位数，不能把各分项中位数直接相加。', '',
             'A0→A1比较全J解析计算与裁剪到V4保留J计算；A1→A2移除独立J和COO，还改变导数遍历与scalc复用；A2→A3比较两种直接填充实现。不能把A0→A3的总收益全归于COO删除。', '']
    for case in cases:
        group = [r for r in rows if r['case'] == case]
        first = group[0]
        lines += [f'### {case}：组装分项', '',
                  f"四条路径均接受{first['iterations']}步，求解{first['linear_solves']}次。Jacobian评估{first['jacobian_evaluations']}次；A0/A1的COO组装{first['coo_assemblies']}次，A2/A3为0次。", '',
        ...
        final = group[-1]
        lines += ['', f"A3相对A0：组装准备{final['assembly_speedup_vs_a0']:.2f}×，总执行{final['total_speedup_vs_a0']:.2f}×。", '']
    lines += ['CSV：`../data/measurements.csv`保留逐轮原始字段和参数；`../data/report.csv`保存主报告表格；`../data/ablation.csv`保存上述消融分项和加速比。最小/最大耗时也保留在汇总CSV中。', '']
    return lines
...
def write_latex(root, stages, cases):
    ...
    out = [r'\begin{table*}[t]', r'\centering', r'\caption{Upper-triangular KKT assembly ablation. Median cumulative times in ms over seven solves; A0 evaluates and slices a full analytical Jacobian without V4; A1 uses V4 plus COO; A2 and A3 are the original and operator direct fills.}',
    ...
    out = [r'\begin{table}[t]', r'\centering', r'\caption{A3/A0 comparison expressed as baseline time divided by final time. Counts include all accepted steps and linear solves.}',
    ...
    r = next(r for r in stages if r['case'] == 'pegase9241' and r['stage'] == 'A3')
...
BACKENDS = {
    '正规方程-QDLDL': 'V4 + JᵀJ / QDLDL',
    '正规方程-KLU': 'V4 + JᵀJ / KLU',
    '增广方程-完整-QDLDL': '算子完整增广 / QDLDL',
    '增广方程-完整-KLU': '算子完整增广 / KLU',
    '增广方程-上三角-QDLDL': '算子上三角 / QDLDL',
}
BACKEND_TIMES = ['build_ms', 'fill_ms', 'product_ms', 'mu_ms', 'matrix_preparation_ms',
                 'analysis_ms', 'factor_ms', 'backsolve_ms', 'linear_ms', 'solve_ms', 'total_execution_ms']


def backend_summaries(records, cases):
    result = []
    for case in cases:
        for method in BACKENDS:
            rows = [r for r in records if r['case'] == case and r['method'] == method and r['round'] > 0]
            first = rows[0]
            out = {k: first[k] for k in ['case', 'method', 'backend', 'matrix_storage', 'jacobian_method']}
            out.update(measured_runs=len(rows), converged_runs=sum(r['converged'] for r in rows),
                       residual_inf_max=max(r['residual_inf'] for r in rows))
            for key in BACKEND_TIMES:
                out[key] = median(r[key] for r in rows)
            for key in ['iterations', 'linear_solves', 'total_execution_ms']:
                out[key + '_min'] = min(r[key] for r in rows)
                out[key + '_max'] = max(r[key] for r in rows)
            for key in ['first_factor_count', 'refactor_count', 'factor_fallback_count']:
                values = [r[key] for r in rows if key in r]
                out[key + '_min'] = min(values) if values else None
                out[key + '_max'] = max(values) if values else None
            flatten('options', first['options'], out)
            result.append(out)
    return result


def write_latex_backends(root, rows):
    labels = {'正规方程-QDLDL': 'NE / QDLDL', '正规方程-KLU': 'NE / KLU',
              '增广方程-完整-QDLDL': 'Full aug. / QDLDL', '增广方程-完整-KLU': 'Full aug. / KLU',
              '增广方程-上三角-QDLDL': 'Upper aug. / QDLDL'}
    out = [r'\begin{table*}[t]', r'\centering',
           r'\caption{KLU and QDLDL on normal equations and augmented systems. Median times in ms over seven measured solves. Normal equations use V4; augmented paths use the operator. KLU receives full CSC. Failed runs, if any, are reported as such rather than ranked by runtime.}',
           r'\label{tab:backends}', r'\begin{tabular}{llrrrrrr}', r'\toprule',
           r'Case & Method & Success & Steps/solves & Preparation & Factorization & Linear total & Execution \\', r'\midrule']
    for r in rows:
        def span(key):
            low, high = r[key + '_min'], r[key + '_max']
            return str(low) if low == high else f'{low}--{high}'
        out.append(r['case'].replace('_', r'\_') + ' & ' + labels[r['method']] +
                   f" & {r['converged_runs']}/{r['measured_runs']} & {span('iterations')}/{span('linear_solves')} & " +
                   ' & '.join(f'{r[k]:.3f}' for k in ['matrix_preparation_ms', 'factor_ms', 'linear_ms', 'total_execution_ms']) + r' \\')
    out += [r'\bottomrule', r'\end{tabular}', r'\end{table*}']
    (root / 'paper/tables/backends.tex').write_text('\n'.join(out) + '\n')
